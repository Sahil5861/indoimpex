        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    //                 <a href="#" onclick="editUser(this)" data-id="'.$row->id.'" data-type="'.$row->job_type.'" data-value="'.$row->type_value.'"class="dropdown-item">
                    //                     <i class="ph-pencil me-2"></i>Edit
                    //                 </a>                                    
                                    // <a href="' . route('jobtypes.remove', $row->id) . '" data-id="' . $row->id . '" class="dropdown-item delete-button">
                    ...
                    if (hasPermission('Job Type Update', 'Update')) {
                        $action .= '<a href="#" onclick="editUser(this)" data-id="'.$row->id.'" data-type="'.$row->job_type.'" data-value="'.$row->type_value.'"class="dropdown-item">
                                        <i class="ph-pencil me-2"></i>Edit
                                    </a>';
                    }
                    if (hasPermission('Job Type Delete', 'Delete')) {
                        $action .= '<a href="#" data-url="' . route('jobtypes.remove', $row->id) . '" data-id="' . $row->id . '" class="dropdown-item delete-button">
                                        <i class="ph-trash me-2"></i>Delete
                                    </a>';
                    }
                    $action .= '</div></div>';      
